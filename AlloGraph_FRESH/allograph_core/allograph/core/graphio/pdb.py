...
def pdb_to_rin(
    ...
    try:
        ...
        raise ImportError(
            "Biopython is required for pdb_to_rin. Install with: pip install biopython"
        ) from e
    ...
    for ch in model:
        ...
        for res in ch:
            # Skip hetero/water residues: Bio.PDB uses id tuple (" ", resseq, icode)
    ...
    chain_label = chain if chain is not None else "all"
    return GraphBundle(A=A, meta=meta, name=f"rin({chain_label};cutoff={cutoff})")
...
def available_chains(pdb_path: str) -> List[str]:
    try:
        ...
        raise ImportError(
            "Biopython is required for available_chains. Install with: pip install biopython"
        ) from e
